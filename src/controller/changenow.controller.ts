...
export class ChangeNowController {
    ...
    async getAvailableCurrencies(request: FastifyRequest, reply: FastifyReply) {
        try {
            ...
            const result = await this.changeNowService.getAvailableCurrencies(options);
     ...
     */
    async getEstimatedExchangeAmount(request: FastifyRequest, reply: FastifyReply) {
        try {
            const query = request.query as any;
            ...
            if (query.useRateId !== undefined) options.useRateId = query.useRateId === 'true';
            if (query.isTopUp !== undefined) options.isTopUp = query.isTopUp === 'true';
    ...
    async getTransactionStatus(request: FastifyRequest, reply: FastifyReply) {
        try {
            ...
            const result = await this.changeNowService.getTransactionStatus(params.id);
    ...
    async getUserAddresses(request: FastifyRequest, reply: FastifyReply) {
        try {
            ...
            const result = await this.changeNowService.getUserAddresses(query.name, query.apiKey);
    ...
    async createFiatTransaction(request: FastifyRequest, reply: FastifyReply) {
        try {
            ...
            if (!body.from_currency || !body.from_amount || !body.to_currency || !body.to_amount || !body.to_address) {
                return reply.status(400).send({
                    success: false,
                    error: 'from_currency, from_amount, to_currency, to_amount, and to_address are required'
                });
            }
    ...
    async getFiatTransactionStatus(request: FastifyRequest, reply: FastifyReply) {
        try {
            ...
            const result = await this.changeNowService.getFiatTransactionStatus(params.id);
    ...
    async getFiatEstimate(request: FastifyRequest, reply: FastifyReply) {
        try {
            ...
            const result = await this.changeNowService.getFiatEstimate(estimateRequest);
     ...
     */
    async getCryptoCurrenciesForFiat(request: FastifyRequest, reply: FastifyReply) {
        try {
